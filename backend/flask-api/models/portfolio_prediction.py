...
def predict_portfolio_risk(stock_file_mapping, portfolio_stocks, forecast_days=30, confidence_level=0.95):
    ...
    for stock in portfolio_stocks:
        ...
        if not symbol or symbol not in stock_file_mapping:
            ...
            if not found:
                print(f"No data file found for {raw_symbol}. Available stocks: {list(stock_file_mapping.keys())}")
                continue
        ...
        try:
            ...
            # Check if 'Close' column exists, if not try to find it
            if 'Close' not in stock_df.columns:
                # Try to find the right price column
                price_columns = [col for col in stock_df.columns if 'close' in col.lower() or 'price' in col.lower()]
                if price_columns:
                    print(f"Using column '{price_columns[0]}' as Close price")
                    stock_df['Close'] = stock_df[price_columns[0]]
                else:
                    # Try to use the 3rd column (common for CSV price data)
                    if len(stock_df.columns) >= 3:
                        print(f"Using column '{stock_df.columns[2]}' as Close price")
